...
- 11:12 - Migration complète de l’infrastructure vers le nouveau compte Vercel/GitHub : liaison locale sur le projet `stavnet` (scope `stav-net`), création du nouveau Blob public `stavnet-media` (`store_71vPaj9AM1s1XzqC`), propagation du `BLOB_READ_WRITE_TOKEN` sur production/preview/development, copie intégrale des 37 médias du store source vers le nouveau store en conservant les mêmes pathnames, déploiement de production et alias actif sur `stavnet.vercel.app`.
- 11:34 - Finalisation de l’indépendance vis-à-vis de l’ancien compte : remplacement en production D1 de l’ancien host Blob (`m44xvdoxrimpqag9.public.blob.vercel-storage.com`) par le nouveau (`71vpaj9am1s1xzqc.public.blob.vercel-storage.com`) dans `books.image_key`, `books.payload` et `books.search_text` (32 notices mises à jour, 0 référence restante), puis bascule du remote Git `origin` vers le fork `mickael-stavnet/Stavnet`.
