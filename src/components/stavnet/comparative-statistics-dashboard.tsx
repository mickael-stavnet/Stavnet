...
export function ComparativeStatisticsDashboard({ items, coverage, compact = false, labels }: ComparativeStatisticsDashboardProps) {
  ...
  const chartHeight = compact ? "h-[250px] md:h-[282px]" : "h-[390px] md:h-[440px]";
  const tableHeight = compact ? "min-h-[250px] md:min-h-[282px]" : "min-h-[390px] md:min-h-[440px]";
  ...
  const knownCoverage = currentSlide?.id === "timeline" ? coverage?.timeline : currentSlide?.id === "languages" ? coverage?.languages : currentSlide?.id === "countries" ? coverage?.countries : currentSlide?.id === "roles" ? coverage?.roles : undefined;
  ...
  return <section className={`${compact ? "mt-4 p-3" : "mt-5 p-4"} border border-[#6295a9] bg-[#cce8f1]/90 shadow-[2px_2px_0_rgba(0,43,112,0.2)]`}>
