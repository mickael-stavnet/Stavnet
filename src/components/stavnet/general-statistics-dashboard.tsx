...
import type { ComparisonCandidatePage } from "@/actions/load-comparison-candidates";
...
interface GeneralStatisticsDashboardProps {
  ...
  countryTimeline: GeneralStatisticsResult["timeline"];
  translationLanguageTimeline: GeneralStatisticsResult["timeline"];
  basePath: string;
  ...
  labels: {
    ...
    compareRecords: string; compareDescription: string; books: string; persons: string; organizations: string; search: string; selected: string; maximum: string; validate: string; clearSelection: string; noResults: string;
    ...
    timelineTitle: string; countryPublicationsTitle: string; translationLanguageTitle: string; originalAuthorsTitle: string; originalBooksTitle: string; translatedBooksTitle: string; translatedAuthorsTitle: string; originalPublishersTitle: string; translatorsTitle: string; translationPublishersTitle: string; pocketReissuesTitle: string; languagesTitle: string; countriesTitle: string; genresTitle: string; birthCountriesTitle: string;
    countryPublicationsScope: string; translationLanguageScope: string;
  };
}
...
function FocusedTimelineSlide({ data, metric, label, noData }: { data: GeneralStatisticsResult["timeline"]; metric: "publications" | "translations"; label: string; noData: string }) {
  const rows = timelineByDecade(data).map((entry) => ({
    period: entry.period,
    value: metric === "translations" ? entry.translations : entry.originals + entry.translations,
  })).filter((entry) => entry.value > 0);
  if (rows.length === 0) return <p className="flex min-h-[280px] items-center justify-center px-5 py-10 text-center text-base text-[#315565] sm:min-h-[360px] lg:min-h-[470px]">{noData}</p>;
  return <ChartContainer config={{ value: { label, color: rankingBarColors[0] } }} className="h-[280px] w-full aspect-auto px-2 py-3 sm:h-[360px] sm:px-5 sm:py-5 lg:h-[clamp(340px,30vh,470px)] lg:px-12"><BarChart data={rows} barCategoryGap="20%" margin={{ top: 24, left: 0, right: 12, bottom: 8 }}><CartesianGrid vertical={false} /><XAxis dataKey="period" tickLine={false} axisLine={false} tickMargin={8} tick={{ fontSize: 11 }} interval="preserveStartEnd" minTickGap={24} /><YAxis allowDecimals={false} tickLine={false} axisLine={false} tick={{ fontSize: 11 }} width={42} /><ChartTooltip content={<ChartTooltipContent />} /><Bar dataKey="value" maxBarSize={64} radius={[4, 4, 0, 0]}>{rows.map((entry, index) => <Cell key={entry.period} fill={rankingBarColors[index % rankingBarColors.length]} />)}<LabelList dataKey="value" position="top" fill="#07384a" fontSize={11} /></Bar></BarChart></ChartContainer>;
}
...
export function GeneralStatisticsDashboard({ data, filters, countryTimeline, translationLanguageTimeline, basePath, comparisonPath, candidates, candidatePages, labels }: GeneralStatisticsDashboardProps) {
  ...
  const slides = useMemo(() => [
    ...
    { id: "country-publications", title: labels.countryPublicationsTitle, filters: ["country"] as FilterKey[], content: <FocusedTimelineSlide data={countryTimeline} metric="publications" label={labels.works} noData={labels.noData} /> },
    { id: "translation-language", title: labels.translationLanguageTitle, filters: ["language"] as FilterKey[], content: <FocusedTimelineSlide data={translationLanguageTimeline} metric="translations" label={labels.translations} noData={labels.noData} /> },
    ...
    { id: "original-books", title: labels.originalBooksTitle, filters: ["period", "language", "country", "publisher"] as FilterKey[], content: <RankingSlide title={labels.originals} data={data.rankings.originalBooks} labels={labels} /> },
  ...
  ], [countryTimeline, data, labels, translationLanguageTimeline]);
  const current = slides[activeSlide] ?? slides[0];
  const currentFilters = current?.filters ?? [];
  const currentScope = current?.id === "country-publications"
    ? labels.countryPublicationsScope.replace("{country}", filters.country || labels.allCountries)
    : current?.id === "translation-language"
      ? labels.translationLanguageScope.replace("{language}", filters.language || labels.allLanguages)
      : current?.id === "pocket"
        ? labels.pocketScope
        : current?.id === "birth-countries"
          ? labels.birthCountriesScope
          : fullScope;
  ...
  return <section className="w-full max-w-[1480px] px-0 py-2 sm:py-4 lg:px-2">
    <div className="flex flex-col gap-3 border-b border-[#6295a9] pb-3 sm:flex-row sm:items-center sm:justify-between"><h2 className="text-xl font-bold text-[#002b9e] sm:text-[23px]">{labels.dashboard}</h2><Dialog><DialogTrigger asChild><Button type="button" variant="outline" className="h-11 w-full rounded-none border-[#456f87] bg-[#fffbd1] px-4 text-sm font-bold text-[#07384a] hover:bg-[#dceff5] hover:text-[#002b9e] sm:w-auto sm:px-5 sm:text-[15px]">{labels.compareRecords}</Button></DialogTrigger><DialogContent className="flex max-h-[calc(100dvh-1rem)] w-[min(96vw,1120px)] max-w-none flex-col gap-0 overflow-hidden border-[#456f87] bg-[#eaf6fa] p-0 sm:max-h-[min(760px,calc(100dvh-3rem))] sm:max-w-none"><DialogHeader className="shrink-0 border-b border-[#6295a9] px-4 py-4 pe-12 text-start sm:px-5"><DialogTitle className="text-xl text-[#002b9e]">{labels.compareRecords}</DialogTitle><DialogDescription className="mt-1 text-sm leading-relaxed text-[#315565]">{labels.compareDescription}</DialogDescription></DialogHeader><ComparativeStatisticsSelector candidates={candidates} candidatePages={candidatePages} initialType="books" initialIds={[]} comparisonPath={comparisonPath} labels={{ books: labels.books, persons: labels.persons, organizations: labels.organizations, search: labels.search, selected: labels.selected, maximum: labels.maximum, validate: labels.validate, clearSelection: labels.clearSelection, noResults: labels.noResults }} /></DialogContent></Dialog></div>
