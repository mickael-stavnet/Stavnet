...
export function StatisticsExplorer({ filters, result, basePath, candidates, initialIds, comparisonPath, labels }: StatisticsExplorerProps) {
  ...
  const candidatePages: Record<ComparisonType, { page: number; total: number; totalPages: number }> = {
    books: { page: 1, total: candidates.books.length, totalPages: 1 },
    persons: { page: 1, total: candidates.persons.length, totalPages: 1 },
    organizations: { page: 1, total: candidates.organizations.length, totalPages: 1 },
  };
