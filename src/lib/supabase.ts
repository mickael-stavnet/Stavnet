...
class Query<T extends Record<string, unknown>> implements PromiseLike<DataClientResult<T[]>> {
  ...
  maybeSingle(): Promise<DataClientResult<T | null>> { this.expectSingle = true; return this.executeSingle(); }
  single(): Promise<DataClientResult<T>> { this.expectSingle = true; return this.executeSingle() as Promise<DataClientResult<T>>; }
...
export const supabase = {
  from<T extends Record<string, unknown>>(table: string): { select(columns?: string, options?: { count?: "exact"; head?: boolean }): Query<T> } { return { select: (columns = "*", options) => { const query = new Query<T>(table, options?.count === "exact"); return query.select(columns, options); } }; },
