...
class Query<T extends Record<string, unknown>> implements PromiseLike<DataClientResult<T[]>> {
  private readonly filters: Filter[] = []; private readonly orders: Order[] = []; private from = 0; private to = 999; private head = false; private columns = "*";
  constructor(private readonly table: string, private readonly wantsCount = false) {}
  select(columns = "*", options?: { count?: "exact"; head?: boolean }): this { this.columns = columns; this.head = options?.head ?? false; return this; }
