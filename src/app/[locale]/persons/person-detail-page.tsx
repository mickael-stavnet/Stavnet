...
function FilledBox({ value, className }: FilledBoxProps) {
  return (
    <div
      className={cn(
        "flex min-h-[42px] items-center border border-[#7aa8b7] bg-[#a7dcee] px-3 text-[14px] font-bold text-black md:min-h-[48px] md:text-[18px]",
        className,
      )}
    >
      {value || "—"}
...
function LabelCell({ label }: { label: string }) {
  return <div className="border border-[#7aa8b7] bg-[#fff8c8] px-3 py-[5px] text-[12px] uppercase leading-none text-black md:text-[13px]">{label}</div>;
}
  ...
  return (
    <main className="relative min-h-[100svh] overflow-x-hidden bg-[#e7f2f7] font-[Arial,Helvetica,sans-serif] text-black md:h-screen md:overflow-hidden">
      ...
      <div className="relative z-10 mx-auto flex min-h-[100svh] w-full max-w-[1120px] flex-col px-4 pb-5 pt-4 md:h-screen md:max-w-none md:px-0 md:pb-0 md:pt-0">
        ...
        <section className="mt-6 flex flex-col gap-4 md:absolute md:left-[3.8vw] md:right-[3.8vw] md:top-[172px] md:bottom-[118px] md:grid md:grid-cols-[96px_1fr_28px] md:gap-[0px]">
          ...
          <section className="order-1 min-w-0 md:order-2 md:mx-auto md:w-full md:max-w-[1320px]">
            <nav className="-mx-1 flex gap-2 overflow-x-auto px-1 pb-2 md:mx-0 md:grid md:grid-cols-[108px_repeat(7,minmax(0,1fr))] md:items-end md:gap-[12px] md:overflow-visible md:px-0 md:pb-0">
              {tabs.map((tabKey) => (
                <button
                  key={tabKey}
                  type="button"
                  ...
                  className={cn(
                    "min-h-11 min-w-[128px] shrink-0 snap-start rounded-t-[8px] border border-[#d1bb48] px-3 py-[8px] text-center text-[13px] leading-[1.02] shadow-[3px_3px_5px_rgba(0,0,0,0.28)] transition-colors md:min-h-[60px] md:min-w-0 md:px-4 md:text-[15px]",
                    activeTab === tabKey ? "bg-[#91d3ea] text-black md:min-h-[68px] md:text-[19px] md:font-bold" : "bg-[#ffea56] text-black hover:bg-[#fff16f]",
                  )}
                >
                  {t(`tabs.${tabKey}`)}
                </button>
            ...
            <div className="mt-[2px] flex min-h-[600px] flex-col rounded-[8px] border border-[#7aa8b7] bg-[linear-gradient(180deg,#8ecfe8_0%,#a8dbed_100%)] shadow-[4px_4px_8px_rgba(0,0,0,0.18)] md:h-[690px] md:min-h-0 md:flex-row">
              ...
              <div className="min-w-0 flex-1 px-[12px] py-[12px] md:px-[16px] md:py-[16px]">
                {activeTab === "authorCard" ? (
                  <div className="grid h-full grid-rows-[auto_auto_minmax(0,1fr)] gap-y-[14px] md:gap-y-[16px]">
                    <div className="grid gap-[12px] lg:grid-cols-[2.05fr_1fr]">
                      <section className="border border-[#7aa8b7] bg-[#a7dcee]">
                        <LabelCell label={t("fields.person")} />
                        <FilledBox value={person.name} className="border-x-0 border-b text-[18px] md:text-[22px]" />
                        ...
                        <div className="grid md:grid-cols-[1.6fr_1fr]">
                          <div>
                            ...
                            <FilledBox value={person.professionalActivity} className="border-x-0 border-b-0" />
                          </div>
                          <div>
                            <LabelCell label="" />
                            <FilledBox value={person.residence || person.language || person.type} className="border-x-0 border-b-0" />
                          </div>
                        </div>
                      </section>
                      ...
                      <section className="border border-[#7aa8b7] bg-[#a7dcee]">
                        ...
                        <div className="grid h-full grid-rows-[48px_repeat(4,1fr)]">
                          <FilledBox value={person.alternateName} className="border-x-0 border-b text-[15px] font-normal md:text-[18px]" />
                    ...
                    <section className="space-y-[8px]">
                      ...
                      <section className="hidden overflow-hidden border border-[#7aa8b7] bg-[#a7dcee] md:block">
                        <div className="grid w-full grid-cols-[0.9fr_1.25fr_2.7fr_0.42fr_0.34fr] border-b border-[#7aa8b7] bg-[#fff8c8] text-[13px] uppercase leading-none text-black">
                          ...
                          <div className="px-3 py-[5px]">{t("bibliography.columns.issue")}</div>
                        </div>

                        {(person.bibliographyRows.length > 0 ? person.bibliographyRows : [{ type: "", language: "", title: "", year: "", issue: "" }]).map(
                          (row, rowIndex) => (
                            <div key={rowIndex} className="grid w-full grid-cols-[0.9fr_1.25fr_2.7fr_0.42fr_0.34fr]">
                              ...
                              <div className="flex h-[42px] items-center border-t border-[#7aa8b7] px-3 text-[15px] text-black">{row.issue}</div>
                            </div>
                          ),
                        )}
                      </section>
          ...
          <aside className="order-3 hidden items-center justify-start md:flex">
            <div className="flex h-full -translate-x-[18px] flex-col items-center justify-between py-[96px] text-[15px] leading-none text-black">
