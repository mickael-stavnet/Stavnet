  ...
  return (
    <main dir="ltr" className="relative min-h-[100svh] overflow-x-hidden bg-[#e7f2f7] font-[Arial,Helvetica,sans-serif] text-black md:h-screen md:overflow-hidden">
      ...
      <div className="relative z-10 mx-auto flex min-h-[100svh] w-full max-w-[1120px] flex-col px-4 pb-5 pt-4 md:h-screen md:max-w-none md:px-0 md:pb-0 md:pt-0">
        ...
        <section className="mt-6 flex min-w-0 flex-col gap-5 md:absolute md:left-1/2 md:top-[160px] md:bottom-[100px] md:w-[1437px] md:max-w-[calc(100vw-24px)] md:-translate-x-1/2 md:box-border md:pl-[282px] md:pr-[35px]">
          <aside className="order-1 flex min-w-0 flex-col gap-[12px] md:absolute md:left-0 md:top-[66px] md:h-[min(660px,calc(100dvh-380px))] md:w-[270px] md:gap-[14px] md:overflow-hidden">
          ...
          <section className="order-2 min-w-0 md:w-full md:max-w-none">
            <nav className="grid grid-cols-2 gap-2 pb-2 md:grid md:w-full md:grid-cols-[1.05fr_1.28fr_1.26fr_0.92fr_1.28fr_1.08fr_0.96fr_1.02fr] md:items-end md:gap-[8px] md:pb-0 [@media(max-width:1440px)]:gap-[6px]">
              {tabs.map((tabKey) => {
                const tabClassName = cn(
                    "min-h-[36px] min-w-0 rounded-t-[8px] border border-[#d1bb48] px-3 py-[8px] text-center text-[13px] font-bold leading-[1.1] shadow-[3px_3px_5px_rgba(0,0,0,0.28)] transition-colors md:flex md:min-h-[48px] md:w-full md:items-center md:justify-center md:px-4 md:text-[15px] md:leading-[1.08] md:whitespace-normal [@media(max-width:1440px)]:min-h-[44px] [@media(max-width:1440px)]:px-2 [@media(max-width:1440px)]:text-[13px]",
                    activeTab === tabKey
            ...
            <div className="mt-[2px] flex min-h-[420px] min-w-0 flex-col rounded-[8px] border border-[#7aa8b7] bg-[linear-gradient(180deg,#8ecfe8_0%,#a8dbed_100%)] shadow-[4px_4px_8px_rgba(0,0,0,0.18)] md:h-[min(660px,calc(100dvh-370px))] md:min-h-0 md:overflow-hidden">
              <div className="min-w-0 flex-1 overflow-y-auto px-3 py-3 md:px-[16px] md:py-[10px]">
                ...
                {activeTab === "statistics" ? <DetailStatisticsPanel kind="book" statistics={book.statistics} labels={{ title: t("content.statistics"), year: statisticsT("year"), decade: statisticsT("decade"), month: statisticsT("month"), monthlyUnavailable: statisticsT("monthlyUnavailable"), noData: statisticsT("noData"), timeline: statisticsT("timeline"), primary: statisticsT("primary"), secondary: statisticsT("secondary"), languages: statisticsT("languages"), countries: statisticsT("countries"), roles: statisticsT("roles"), overview: statisticsT("overview"), period: statisticsT("period"), records: statisticsT("records"), languagesCount: statisticsT("languagesCount"), countriesCount: statisticsT("countriesCount"), rolesCount: statisticsT("rolesCount"), reach: statisticsT("reach"), contributions: statisticsT("contributions"), bookActivity: statisticsT("bookActivity"), personActivity: statisticsT("personActivity"), organizationActivity: statisticsT("organizationActivity") }} /> : null}
              </div>
            </div>
          </section>

          <aside className="order-3 hidden md:absolute md:right-0 md:top-[66px] md:flex md:h-[min(660px,calc(100dvh-380px))] md:w-[34px] md:items-center md:justify-center">
