    ...
    <section className="rounded-[6px] border border-[#7aa8b7] bg-[#a7dcee] md:hidden">
      ...
      <div className="space-y-3 p-3">
        {rows.map((row, rowIndex) => (
          <div
            key={`${title}-${rowIndex}`}
            className="space-y-2 rounded-[4px] border border-[#7aa8b7] bg-[#b2e0ef] p-3"
          >
...
function FilledTable({ columns, data, rows = 1, className }: FilledTableProps) {
  return (
    <section
      className={cn(
        "overflow-x-auto border border-[#7aa8b7] bg-[#a7dcee]",
        className,
      )}
    >
      <div
      ...
      >
        {columns.map((column) => (
          <div
            key={column}
            className="border-r border-[#7aa8b7] px-2 py-[3px] last:border-r-0"
          >
            {column}
          </div>
        ))}
          ...
          <section className="order-2 min-w-0 md:col-start-1 md:col-end-2 md:ml-[282px] md:w-[1120px] md:max-w-none">
            <nav className="flex gap-2 overflow-x-auto pb-2 md:grid md:grid-cols-[92px_repeat(7,minmax(0,1fr))] md:items-end md:gap-[10px] md:overflow-visible md:pb-0">
              {tabs.map((tabKey) =>
                tabKey === "backCover" ? (
                  <Link
                    key={tabKey}
                    href="/books/details/back-cover"
                    className="min-h-[44px] min-w-[150px] shrink-0 rounded-t-[8px] border border-[#d1bb48] bg-[#ffea56] px-3 py-[10px] text-center text-[13px] leading-[1.1] text-black shadow-[3px_3px_5px_rgba(0,0,0,0.28)] transition-colors hover:bg-[#fff16f] md:min-w-0 md:px-2"
                  >
                    {t(`tabs.${tabKey}`)}
                  </Link>
                ) : tabKey === "pressCritiques" ? (
                  <Link
                    key={tabKey}
                    href="/books/details/press-critiques"
                    className="min-h-[44px] min-w-[150px] shrink-0 rounded-t-[8px] border border-[#d1bb48] bg-[#ffea56] px-3 py-[10px] text-center text-[13px] leading-[1.1] text-black shadow-[3px_3px_5px_rgba(0,0,0,0.28)] transition-colors hover:bg-[#fff16f] md:min-w-0 md:px-2"
                  >
                    {t(`tabs.${tabKey}`)}
                  </Link>
                  ...
                  <Link
                    key={tabKey}
                    href="/books/details/availability"
                    className="min-h-[44px] min-w-[150px] shrink-0 rounded-t-[8px] border border-[#d1bb48] bg-[#ffea56] px-3 py-[10px] text-center text-[13px] leading-[1.1] text-black shadow-[3px_3px_5px_rgba(0,0,0,0.28)] transition-colors hover:bg-[#fff16f] md:min-w-0 md:px-2"
                  >
                    {t(`tabs.${tabKey}`)}
                  </Link>
            ...
            <div className="mt-[2px] flex min-h-[420px] min-w-0 flex-col rounded-[8px] border border-[#7aa8b7] bg-[linear-gradient(180deg,#8ecfe8_0%,#a8dbed_100%)] shadow-[4px_4px_8px_rgba(0,0,0,0.18)] md:h-[660px] md:flex-row">
              <aside className="border-b border-[#7aa8b7] px-3 py-4 md:w-[128px] md:border-b-0 md:border-r">
                <p className="text-[18px] font-bold leading-tight text-[#ff1313]">
                  {t("side.translation")}
                </p>
                <p className="mt-[2px] text-[16px] font-bold leading-tight text-black">
                  {t("side.language")}
                </p>
              </aside>
              ...
              <div className="min-w-0 flex-1 overflow-y-auto px-3 py-3 md:px-[16px] md:py-[10px]">
                  ...
                  <div className="grid h-full gap-y-[14px] md:grid-rows-[auto_auto_auto_1fr]">
                    ...
                    <div className="space-y-[14px]">
                      ...
                      <MobileDataSection
                        ...
                        columns={[
                          t("tables.contributors"),
                          t("tables.contributionType"),
                          t("tables.writingLanguage"),
                        ]}
                    ...
                    <div className="grid gap-[12px] self-end sm:grid-cols-2 md:grid-cols-4">
                      <MiniCard
                        ...
                        values={sampleBook.category}
                      />
                      <MiniCard
                        title={t("tables.subject")}
                        values={sampleBook.subject}
                      />
                      <MiniCard
                        title={t("tables.gender")}
                        values={sampleBook.gender}
                      />
                      <MiniCard
                        title={t("tables.targetAudience")}
                        values={sampleBook.targetAudience}
                      />
                    </div>
                  </div>
                ) : null}
                ...
                {activeTab === "backCover" ? (
                  <BlankContent title={t("content.backCover")} rows={1} />
                ) : null}
                {activeTab === "tableOfContents" ? (
                  <BlankContent title={t("content.tableOfContents")} rows={3} />
                ) : null}
                {activeTab === "extracts" ? (
                  <BlankContent title={t("content.extracts")} rows={3} />
                ) : null}
                {activeTab === "pressCritiques" ? (
                  <BlankContent title={t("content.pressCritiques")} rows={3} />
                ) : null}
                {activeTab === "availability" ? (
                  <BlankContent title={t("content.availability")} rows={2} />
                ) : null}
                {activeTab === "publishing" ? (
                  <BlankContent title={t("content.publishing")} rows={2} />
                ) : null}
          ...
          <aside className="order-3 hidden items-start justify-start md:absolute md:right-0 md:top-[78px] md:flex">
            <div className="flex flex-col items-center gap-[72px] text-[14px] leading-none text-black">
              <span className="[writing-mode:vertical-rl]">
                {t("right.bookCardsFound")}
              </span>
              <span className="[writing-mode:vertical-rl]">
                {t("right.database")}
              </span>
              <span className="[writing-mode:vertical-rl] text-[#ff1d1d]">
                {t("right.records")}
              </span>
            </div>
          </aside>
