...
export default function BookBackCoverPage({ book }: BookBackCoverPageProps) {
  ...
  return (
    <BookDetailSecondaryLayout book={book} pageName={t("tabs.backCover")} pagePath="/books/details/back-cover" layout={book.backCover ? "default" : "expandedCentered"}>
      {book.backCover ? <section className="flex min-h-0 flex-col border border-[#7ea8b8] bg-[#a6d9eb] md:h-full md:flex-1 md:overflow-hidden">
        ...
        <div className="flex min-h-0 flex-1 items-center px-3 py-[12px] text-[15px] leading-[1.48] text-black md:overflow-y-auto md:px-8 md:py-5 md:text-[17px] md:leading-[1.42]">
          <p className="mx-auto w-full max-w-[1040px]">{book.backCover}</p>
        </div>
      </section> : null}
